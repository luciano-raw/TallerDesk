...
export function AuthProvider({ children, dbUser }: { children: React.ReactNode; dbUser?: any }) {
  ...
  if (!demoMode && dbProfile?.tallerStatus === "PAUSADO" && !dbProfile?.roles?.includes("SUPER_ADMIN")) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        <div className="absolute top-4 right-4">
          <OriginalClerkProvider>
        ...
        <div className="w-20 h-20 rounded-3xl bg-warning/10 text-warning flex items-center justify-center mb-6 shadow-sm">
        ...
        <h1 className="text-3xl font-extrabold mb-3">Suscripcin Pausada</h1>
        <p className="text-base text-muted-foreground max-w-md mb-8 leading-relaxed">
          El acceso a este taller se encuentra suspendido temporalmente por el administrador del sistema.
          <br/><br/>Si eres el dueo, por favor contacta a soporte para reactivar tu cuenta.
        </p>
      </div>
    );
  }
